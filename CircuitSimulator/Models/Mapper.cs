...
using System.Linq;
using Button = CircuitSimulator.Views.Shapes.Button;
        ...
        */
        ...
        public void FixItem(ref Control res, Point pos, IEnumerable<ILogical> items) {
            foreach (var logic in items) {
                ...
                // if (tb != null && new Rect(tb.Value.Clip.TopLeft, new Size()).Sum(item.Bounds).Contains(pos) && (string?) item.Tag != "Join") res = item; 
        ...
        public int Release(Control item, Point pos, bool use_fix = true) {
            ...
            case 8:
                ...
                if (marker_circle != null && @join != null) {
                    var gate = GetGate(marker_circle) ?? throw new Exception("Что?"); // Такого не бывает
